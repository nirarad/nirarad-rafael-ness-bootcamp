...
class TestNonFunc:
    ...
    def test_security(self):
        ...
        try:
            ...
            if status_waiting(6) is True:
                raise ValueError("access with bob account available, change to status 6")
        except AssertionError as ae:
            self.log.send(
                self.config["ASSERT_FAIL"].format("test_security", "UserCheckoutAcceptedIntegrationEvent", ae))
            assert False
        except Exception as e:
            self.log.send(
                self.config["TEST_FAIL"].format("test_security", "UserCheckoutAcceptedIntegrationEvent", e))
            assert False
    ...
    def test_scalability(self):
        try:
            ...
            self.log.send(
                self.config["ASSERT_FAIL"].format("test_scalability", "UserCheckoutAcceptedIntegrationEvent", ae))
            assert False
        except Exception as e:
            self.log.send(
                self.config["TEST_FAIL"].format("test_scalability", "UserCheckoutAcceptedIntegrationEvent", e))
            assert False
        finally:
    ...
    def test_reliability(self):
        """
        Name: menahem rotblat\n
        Description: tests Orders reliability by shut down the Ordering Api service and send to it msg to continue with
        ...
        try:
            ...
            # payment get msg from orders
            self.payment.consume()
            self.log.send(
                self.config["TEST_PASS"].format("test_reliability", "OrderStockConfirmedIntegrationEvent",
                                                "3"))
            ...
            self.log.send(
                self.config["TEST_PASS"].format("test_reliability",
                                                "OrderStatusChangedToAwaitingValidationIntegrationEvent",
                                                self.signalrhub.routing_key_srh_get))
        except AssertionError as ae:
            self.log.send(
                self.config["ASSERT_FAIL"].format("test_reliability", "UserCheckoutAcceptedIntegrationEvent",
                                                  ae))
            assert False
        except Exception as e:
            self.log.send(
                self.config["TEST_FAIL"].format("test_reliability", "UserCheckoutAcceptedIntegrationEvent", e))
            assert False
        finally:
            self.dm.containers_dict["eshop/ordering.signalrhub:linux-latest"].start()
            self.dm.containers_dict["eshop/ordering.api:linux-latest"].start()
